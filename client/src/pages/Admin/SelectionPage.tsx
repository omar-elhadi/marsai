...
export default function SelectionPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/awards/selection`, {
        credentials: "include",
      });
      ...
      setFilms(
        Array.isArray(data)
          ? data.filter((f: any) =>
              ["APPROVED", "SELECTION"].includes(f.status),
            )
          : [],
      );
    } finally {
        ...
        >
          <div>
            ...
            <p className="body-meta">
              Films approuvés par le jury — à sélectionner pour la compétition.
            </p>
          </div>
          ...
          {inSelection.length > 0 && (
            ...
            >
              <Trophy size={12} />
              {inSelection.length} film{inSelection.length > 1 ? "s" : ""} prêt
              {inSelection.length > 1 ? "s" : ""} à nominer → Palmarès
            </Link>
          )}
        </div>
      </header>
                ...
                {visibleFilms.map((film) => (
                  ...
                  >
                    <td style={{ padding: "1rem" }}>
                      <span
                        className={`text-[11px] px-2 py-0.5 border font-bold uppercase tracking-wider ${STATUS_BG[film.status] ?? ""}`}
                      >
