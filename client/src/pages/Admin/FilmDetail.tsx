...
function FilmDetail() {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      ...
      setFilm((prev: any) => ({ ...prev, status: updated.status }));
    } finally {
      setUpdating(false);
    }
  };
  ...
  const nextOptions =
    NEXT_STATUSES[film.status as keyof typeof NEXT_STATUSES] ?? [];
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            >
              Ce message sera envoyé par email au réalisateur avec un lien
              valable 7 jours.
            </p>
        ...
        >
          <div>
            ...
            >
              ...
              <span
                className={`text-xs font-black uppercase px-2 py-1 border shrink-0 ${STATUS_STYLES[film.status as keyof typeof STATUS_STYLES] ?? ""}`}
              >
                {film.status}
              </span>
            </div>
            ...
            >
              {film.country}
              {film.language ? ` · ${film.language}` : ""} · soumis le{" "}
              ...
              <div className="space-y-3">
                {votes.map((vote: any) => (
                  ...
                  >
                    {/* Sentiment */}
                    <div
                      className={`flex-shrink-0 p-1.5 ${vote.sentiment === "LIKE" ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"}`}
                    >
                      {vote.sentiment === "LIKE" ? (
                        <ThumbsUp size={14} />
                      ) : (
        ...
        <div className="space-y-6">
          ...
          {film.modificationRequest &&
            (() => {
              ...
              const submitterResponded =
                lastVersion &&
                film.modificationRequestedAt &&
                new Date(lastVersion.archivedAt) >
                  new Date(film.modificationRequestedAt);
                    ...
                    {submitterResponded ? (
                      <>
                        ...
                        <span className="text-[11px] font-bold uppercase tracking-wider text-green-400">
                          Réalisateur a répondu le{" "}
                          {new Date(lastVersion.archivedAt).toLocaleDateString(
                            "fr-FR",
                          )}
                        </span>
                      </>
                      ...
                      {changedFields.map(({ key, label }) => (
                        <div key={key}>
                          ...
                          <div className="bg-red-500/5 border border-red-500/15 px-2.5 py-2 mb-1">
                            <span className="text-xs font-bold text-red-400 uppercase block mb-0.5">
                              Avant
                            </span>
                            <p className="text-xs text-red-300 leading-relaxed wrap-break-word line-clamp-3">
                              {lastVersion[key] || (
                                <em className="opacity-40">vide</em>
                              )}
                            </p>
                          </div>
                          {/* Après */}
                          <div className="bg-green-500/5 border border-green-500/15 px-2.5 py-2">
                            <span className="text-xs font-bold text-green-400 uppercase block mb-0.5">
                              Après
                            </span>
