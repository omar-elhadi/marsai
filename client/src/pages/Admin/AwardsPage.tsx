...
function Rating({ avg, total }: { avg?: number | null; total?: number }) {
  ...
  return (
    ...
    >
      {avg.toFixed(1)}{" "}
      <span style={{ color: "var(--color-text-faint)" }}>/ {total}v</span>
    </span>
  );
}
...
function TabCategories({
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/awards/categories?edition=${edition}`, {
  ...
  return (
    <div
      style={{
        display: "grid",
        gridTemplateColumns: isAdmin ? "18rem 1fr" : "1fr",
        gap: "2rem",
        alignItems: "start",
      }}
    >
      ...
      <div>
        {categories.length === 0 ? (
          ...
          >
            ...
            {isAdmin ? "Créez-en une ci-contre." : ""}
          </p>
        ) : (
          <div
            style={{
          ...
          >
            <table className="w-full text-left border-collapse">
              <thead>
                ...
                >
                  {["Catégorie", "Nominés", isAdmin ? "Actions" : ""].map(
                    (h, i) => (
                      <th
                        key={i}
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [catRes, selRes] = await Promise.all([
        fetch(`${API}/awards/categories?edition=${edition}`, {
          headers: JSON_HEADERS,
          credentials: "include",
        }),
        fetch(`${API}/awards/selection`, {
          headers: JSON_HEADERS,
          credentials: "include",
        }),
      ]);
      ...
      setSelection(
        Array.isArray(sel) ? sel.filter((f) => f.status === "SELECTION") : [],
      );
    } finally {
      setLoading(false);
    }
        ...
        return (
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <select
                  ...
                  onChange={(e) =>
                    setSelectedFilm((p) => ({ ...p, [cat.id]: e.target.value }))
                  }
                  style={{
                    flex: 1,
                    background: "var(--color-surface)",
            ...
            {cat.nominations.length === 0 ? (
              ...
              >
                Aucun nominé pour cette catégorie.
              </p>
            ) : (
              <table className="w-full text-left border-collapse">
                <thead>
                      ...
                      <td style={{ padding: "0.75rem 1rem" }}>
                        ...
                        <div
                          style={{
                            ...
                            color: "var(--color-text-muted)",
                            marginTop: "0.125rem",
                            paddingLeft: nom.isWinner ? "1.375rem" : "0",
                          }}
                        >
                          {nom.film.country}
                        </div>
        ...
        return (
          ...
          >
            ...
            >
              ...
              <p className="label-overline" style={{ flex: 1 }}>
                {cat.name}
              </p>
              {winner ? (
                <span className="text-[11px] px-2 py-0.5 bg-amber-500/15 text-amber-400 border border-amber-500/25 font-bold uppercase tracking-wider">
                  Primé
                </span>
              ) : (
                <span
                ...
                >
                  {cat.nominations.length} nominé
                  {cat.nominations.length !== 1 ? "s" : ""}
                </span>
              )}
            </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            {summary.nominations > summary.winners &&
              summary.nominations > 0 && (
                <button
                ...
                >
                  <Crown size={12} />
                  {summary.nominations - summary.winners} nomination
                  {summary.nominations - summary.winners > 1 ? "s" : ""} sans
                  primé → Gagnants
                </button>
              )}
      ...
      >
        ...
        >
          <p className="text-2xl font-black" style={{ color: "#818cf8" }}>
            {summary.categories}
          </p>
      ...
      {/* ── Contenu ── */}
      {tab === "categories" && (
        <TabCategories edition={edition} userRole={userRole} />
      )}
