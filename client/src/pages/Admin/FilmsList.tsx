...
function FilmsList() {
  ...
  const [assignPopup, setAssignPopup] = useState<any>(null);
  ...
  const fetchFilms = useCallback(async () => {
    ...
    try {
      ...
      setFilms(
        Array.isArray(data)
          ? data.filter((f) => EVAL_STATUSES.has(f.status))
          : [],
      );
    } catch {
      setFilms([]);
  ...
  useEffect(() => {
    const fetchJury = async () => {
      try {
        ...
        const data = await res.json();
        setJuryUsers(
          Array.isArray(data) ? data.filter((u) => u.role === "JURY") : [],
        );
      } catch {
        setJuryUsers([]);
      }
  ...
  const handleStatusChange = async (film: any, newStatus: string) => {
    setUpdating(film.id);
    try {
  ...
  const openAssignPopup = (film: any) => {
    setAssignError("");
    setAssignPopup({
      filmId: film.id,
      filmTitle: film.title,
      filmStatus: film.status,
      selectedIds: (film.assignedUsers ?? []).map((u: any) => u.id),
    });
  };
  ...
  const toggleJury = (userId: number) => {
    setAssignPopup((prev: any) => ({
      ...prev,
      selectedIds: prev.selectedIds.includes(userId)
        ? prev.selectedIds.filter((id: number) => id !== userId)
        ...
        >
          <div>
            ...
            <p className="body-meta">
              Réception, assignation et modération des films soumis.
            </p>
          </div>
          ...
          {(stats.byStatus?.APPROVED ?? 0) > 0 && (
            ...
            >
              <Star size={12} />
              {stats.byStatus.APPROVED} film
              {stats.byStatus.APPROVED > 1 ? "s" : ""} approuvé
              {stats.byStatus.APPROVED > 1 ? "s" : ""} prêt
              {stats.byStatus.APPROVED > 1 ? "s" : ""} → Sélection
            </Link>
          )}
        </div>
      </header>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: "1rem" }}>
                        <span
                          className={`text-[11px] px-2 py-0.5 border font-bold uppercase tracking-wider ${STATUS_STYLES[film.status] ?? ""}`}
                        >
                          {film.status}
                        </span>
                      </td>
                      ...
                      >
                        {updating === film.id ? (
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              if (e.target.value)
                                handleStatusChange(film, e.target.value);
                            }}
                            style={{
                              background: "var(--color-surface-high)",
                              border: "1px solid var(--color-border)",
      ...
      {assignPopup && (
        ...
        >
          ...
          >
            ...
            {assignPopup.filmStatus === "SUBMITTED" &&
              assignPopup.selectedIds.length > 0 && (
                ...
                >
                  → Film passera en{" "}
                  <strong style={{ marginLeft: "0.25rem" }}>IN_REVIEW</strong>
                </div>
              )}
            {assignPopup.filmStatus === "IN_REVIEW" &&
              assignPopup.selectedIds.length === 0 && (
                <div
                  style={{
                    padding: "0.5rem 1.5rem",
                ...
                >
                  ⚠ Sans jury, le film repassera en{" "}
                  <strong style={{ marginLeft: "0.25rem" }}>SUBMITTED</strong>
                </div>
              )}
            ...
            >
              ...
              <p
                style={{
                  fontSize: "0.6875rem",
                  color: assignError ? "#f87171" : "var(--color-text-faint)",
                  textAlign: "center",
                  marginBottom: "0.75rem",
                  minHeight: "1rem",
                }}
              ...
              <div style={{ display: "flex", gap: "0.75rem" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    justifyContent: "center",
                    gap: "0.5rem",
                    opacity: assigning === assignPopup?.filmId ? 0.5 : 1,
                    transition: "opacity 0.2s",
                  }}
                >
                  {assigning === assignPopup?.filmId ? (
