...
import {
  ArrowLeft, Loader2, MessageSquare, Send, ChevronRight,
  Users, Globe, Cpu, Calendar,
} from "lucide-react";
...
const STATUS_COLORS = {
  SUBMITTED:  { color: 'var(--color-text-muted)',   border: 'var(--color-border)' },
...
export default function JuryFilmDetail() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      setVoteSuccess(wasSuggestion ? "Demande transmise à l'admin ✓" : "Évaluation enregistrée ✓");
  ...
  const handleRemoveVote = async () => {
    setVoting(true);
    try {
      await fetch(`${API}/jury/votes/${id}`, { method: "DELETE", credentials: 'include' });
      setRating(5);
      setSuggestMod({ checked: false, text: "" });
      await fetchFilm();
      await fetchAllFilms();
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      switch (e.key) {
        ...
        case "Enter":     if (!st.voting) { e.preventDefault(); handleSubmitRef.current(); } break;
        case "Escape":    navigate("/jury/dashboard"); break;
        case "ArrowRight":
          if (st.nextFilm) { e.preventDefault(); navigate(`/jury/film/${st.nextFilm.id}`); }
          break;
      }
    };
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--color-bg)', fontFamily: 'var(--font-sans)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-text-faint)' }}>
  ...
  if (error || !film) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--color-bg)', fontFamily: 'var(--font-sans)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem', color: 'var(--color-text-muted)' }}>
        <p style={{ fontSize: '0.875rem' }}>{error || "Film introuvable."}</p>
        <button
          onClick={() => navigate("/jury/dashboard")}
          style={{ fontSize: '0.75rem', color: '#10b981', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}
        >
          Retour au dashboard
        </button>
      </div>
  ...
  const internalComments  = currentVote?.comments?.filter(c => c.isInternal).slice().reverse() ?? [];
  // Commentaire de suggestion (isInternal: false) — transmis à l'admin
  const suggestionComment = currentVote?.comments?.find(c => !c.isInternal) ?? null;
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)', color: 'var(--color-text)', fontFamily: 'var(--font-sans)' }}>
      <div style={{ maxWidth: '72rem', margin: '0 auto', padding: 'clamp(2rem, 4vw, 3rem) clamp(1.5rem, 4vw, 2.5rem)' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                ...
                {currentVote ? (
                  <span style={{
                    fontSize: '0.6875rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em',
                    padding: '0.375rem 0.625rem', flexShrink: 0,
                    background: currentVote.sentiment === "LIKE" ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)',
                    color:      currentVote.sentiment === "LIKE" ? '#34d399' : '#f87171',
                    border:     `1px solid ${currentVote.sentiment === "LIKE" ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)'}`,
                  }}>
                    {currentVote.rating}/10
                  </span>
                ) : (
                  <span style={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)', border: '1px solid var(--color-border)', padding: '0.375rem 0.625rem', textTransform: 'uppercase', letterSpacing: '0.15em', flexShrink: 0 }}>
                    À évaluer
                  </span>
                )}
              </div>
              <p style={{ color: 'var(--color-text-muted)', fontSize: '0.875rem', marginTop: '0.5rem' }}>
                {film.country}{film.language ? ` · ${film.language}` : ''}{film.submittedAt ? ` · soumis le ${new Date(film.submittedAt).toLocaleDateString('fr-FR')}` : ''}
              </p>
            </div>
            ...
            {nextFilm && (
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.625rem', textTransform: 'uppercase', letterSpacing: '0.15em', color: 'var(--color-text-muted)', border: '1px solid var(--color-border)', padding: '0.5rem 0.75rem', background: 'none', cursor: 'pointer', transition: 'color 0.2s, border-color 0.2s', flexShrink: 0 }}
              ...
              >
                <span style={{ maxWidth: '8rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{nextFilm.title}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {youtubeId ? (
              ...
              <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', aspectRatio: '16/9', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-text-faint)', fontSize: '0.875rem' }}>
            ...
            <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', padding: 'clamp(1.25rem, 2.5vw, 1.75rem)' }}>
              ...
              <p style={{ color: 'var(--color-text)', fontSize: '0.875rem', lineHeight: 1.75 }}>{film.description}</p>
            ...
            <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', padding: 'clamp(1.25rem, 2.5vw, 1.75rem)' }}>
              ...
              {currentVote && !isLocked ? (
                <div className="space-y-3">
                  ...
                  {commentError && <p style={{ fontSize: '0.75rem', color: '#f87171' }}>{commentError}</p>}
                ...
                <p style={{ fontSize: '0.8125rem', color: 'var(--color-text-faint)', fontStyle: 'italic' }}>
                  Les notes sont verrouillées — film {STATUS_LABELS[film.status]?.toLowerCase()}.
                </p>
              ) : (
              ...
              {internalComments.length > 0 && (
                <div style={{ borderTop: '1px solid var(--color-border)', paddingTop: '0.75rem', marginTop: '1rem' }} className="space-y-3">
                  {internalComments.map(c => (
                    <div key={c.id} className="space-y-1.5">
                      <p style={{ fontSize: '0.6875rem', color: 'var(--color-text-faint)', fontFamily: 'monospace' }}>
                        {formatDateTime(c.createdAt)}
                      </p>
                      <p style={{ fontSize: '0.875rem', color: 'var(--color-text)', background: 'var(--color-surface-high)', border: '1px solid var(--color-border)', padding: '0.75rem 1rem', lineHeight: 1.75 }}>
                        {c.content}
                      </p>
                    </div>
                  ))}
              ...
              {currentVote && internalComments.length === 0 && (
                <p style={{ fontSize: '0.875rem', color: 'var(--color-text-faint)', fontStyle: 'italic', marginTop: '0.75rem' }}>
          ...
          <div className="space-y-6">
            ...
            {/* Jurys assignés */}
            <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', padding: 'clamp(1.25rem, 2.5vw, 1.5rem)' }}>
              <p className="label-overline" style={{ marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                <Users size={11} /> Jurys assignés
              </p>
              <p style={{ fontSize: '0.875rem', color: 'var(--color-text-muted)' }}>
                {film.assignedUsers?.length ?? 0} évaluateur{(film.assignedUsers?.length ?? 0) > 1 ? "s" : ""}
              </p>
            </div>

            {/* ── Votre évaluation ── */}
            <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', padding: 'clamp(1.25rem, 2.5vw, 1.5rem)' }} className="space-y-4">
              ...
              {isLocked ? (
                ...
                <div className="space-y-3">
                  {currentVote && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', background: 'var(--color-surface-high)', border: '1px solid var(--color-border)' }}>
                      <span className="label-overline">Votre note</span>
                      <span style={{ fontFamily: 'monospace', fontWeight: 900, fontSize: '1.5rem', color: accentColor }}>
                        {currentVote.rating}<span style={{ fontSize: '0.75rem', fontWeight: 400, color: 'var(--color-text-faint)' }}>/10</span>
                      </span>
                    </div>
                  )}
                  <div style={{ padding: '0.625rem 0.875rem', background: 'rgba(251,191,36,0.05)', border: '1px solid rgba(251,191,36,0.2)', textAlign: 'center' }}>
                    <p style={{ fontSize: '0.625rem', color: '#fbbf24', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                      Évaluation clôturée
                    </p>
                    <p style={{ fontSize: '0.6875rem', color: 'var(--color-text-faint)', marginTop: '0.25rem' }}>
                      Film {STATUS_LABELS[film.status]?.toLowerCase()} — modifications non autorisées.
                    </p>
                  </div>
                </div>
              ) : (
                ...
                <>
                  ...
                  <div className="space-y-2">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.375rem' }}>
                        <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: '1.5rem', lineHeight: 1, color: accentColor, transition: 'color 0.2s' }}>
                          {rating}
                          <span style={{ color: 'var(--color-text-faint)', fontSize: '0.875rem', fontWeight: 400 }}>/10</span>
                        </span>
                        <span style={{ fontSize: '0.625rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: accentColor, transition: 'color 0.2s' }}>
                          {sentiment === "LIKE" ? "✓" : "✕"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.6875rem', fontFamily: 'monospace' }}>
                      ...
                      <span style={{ color: 'var(--color-text-faint)' }}>5 | 6</span>
                  ...
                  <button
                    ...
                    disabled={voting}
                    style={{ width: '100%', padding: '0.875rem', fontSize: '0.75rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em', background: '#10b981', color: '#000', border: '1px solid #34d399', cursor: 'pointer', transition: 'background 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}
                    className="disabled:opacity-30"
                    onMouseEnter={e => { if (!e.currentTarget.disabled) e.currentTarget.style.background = '#34d399'; }}
                    onMouseLeave={e => e.currentTarget.style.background = '#10b981'}
                  >
                    {voting
                      ? <Loader2 size={14} className="animate-spin" />
                      : currentVote ? "Enregistrer" : "Soumettre l'évaluation"
                    }
                  </button>
                </>
              )}

              {voteError   && <p style={{ fontSize: '0.75rem', color: '#f87171' }}>{voteError}</p>}
              {voteSuccess && (
                <p style={{ fontSize: '0.75rem', color: '#10b981', fontWeight: 700, textAlign: 'center', letterSpacing: '0.05em' }}>
                  {voteSuccess}
                </p>
              )}
              ...
              {currentVote && !voting && !isLocked && (
                <button
                  ...
                  style={{ width: '100%', fontSize: '0.6875rem', textTransform: 'uppercase', letterSpacing: '0.15em', color: 'var(--color-text-faint)', background: 'none', border: 'none', cursor: 'pointer', padding: '0.25rem', transition: 'color 0.2s', textAlign: 'center' }}
            ...
            {!isLocked && <div style={{ border: '1px solid rgba(249,115,22,0.2)', overflow: 'hidden' }}>
              <div style={{ background: 'rgba(249,115,22,0.05)', padding: '1rem 1.25rem' }} className="space-y-3">
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <p className="label-overline" style={{ color: '#fb923c', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                    <MessageSquare size={11} /> Demande de modification
                  </p>
                  {currentVote?.suggestModification && (
                    <span style={{ fontSize: '0.6875rem', color: '#fb923c', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                      ● Active
                    </span>
                  )}
                </div>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer' }}>
                  <input
                    ...
                    onChange={e => { setSuggestMod({ checked: e.target.checked, text: "" }); setVoteError(""); }}
                    className="accent-orange-500"
                  />
                  <span style={{ fontSize: '0.875rem', color: suggestMod.checked ? '#fb923c' : 'var(--color-text)', transition: 'color 0.2s' }}>
                ...
                {suggestMod.checked && (
                  <>
                    <textarea
                      ...
                      onChange={e => { setSuggestMod(prev => ({ ...prev, text: e.target.value })); setVoteError(""); }}
                      style={{ width: '100%', background: 'transparent', border: '1px solid rgba(249,115,22,0.3)', padding: '0.625rem 0.75rem', fontSize: '0.875rem', color: 'var(--color-text)', outline: 'none', resize: 'none', transition: 'border-color 0.2s', boxSizing: 'border-box' }}
                      className="placeholder:text-text-faint"
                    ...
                    <button
                      ...
                      style={{ width: '100%', padding: '0.75rem', fontSize: '0.75rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em', background: 'rgba(249,115,22,0.15)', color: '#fb923c', border: '1px solid rgba(249,115,22,0.4)', cursor: 'pointer', transition: 'background 0.2s, color 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}
                      className="disabled:opacity-30"
                      onMouseEnter={e => { if (!e.currentTarget.disabled) { e.currentTarget.style.background = 'rgba(249,115,22,0.25)'; e.currentTarget.style.color = '#fdba74'; }}}
                      onMouseLeave={e => { e.currentTarget.style.background = 'rgba(249,115,22,0.15)'; e.currentTarget.style.color = '#fb923c'; }}
                    >
                      {voting ? <Loader2 size={13} className="animate-spin" /> : "Envoyer la demande"}
                    </button>
                  </>
                )}
              </div>
              <div style={{ padding: '0.5rem 1.25rem', borderTop: '1px solid rgba(249,115,22,0.15)', background: 'rgba(0,0,0,0.1)' }}>
                <p style={{ fontSize: '0.6875rem', color: 'rgba(251,146,60,0.6)' }}>
            ...
            {currentVote && (
              <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', padding: '1rem 1.25rem' }}>
                <p className="label-overline" style={{ marginBottom: '1.25rem' }}>Historique</p>
                ...
                <div style={{ position: 'relative', paddingLeft: '2rem' }}>
                  ...
                  {/* Ligne verticale */}
                  <div style={{
                    position: 'absolute', left: '0.5rem', top: '0.625rem', bottom: '0.25rem',
                  ...
                  <div style={{ position: 'relative', marginBottom: '1.25rem' }}>
                    ...
                    <p style={{ fontSize: '0.6rem', color: 'var(--color-text-faint)', fontFamily: 'monospace', letterSpacing: '0.04em' }}>
                      {formatDateTime(currentVote.updatedAt)}
                    </p>
                    <p style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', marginTop: '0.25rem' }}>
                      Évaluation ·{' '}
                      <span style={{ color: accentColor, fontWeight: 700 }}>{currentVote.rating}/10</span>
                      <span style={{ color: 'var(--color-text-faint)', fontSize: '0.625rem', marginLeft: '0.375rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                  ...
                  {suggestionComment && (
                    <div style={{ position: 'relative', marginBottom: '1.25rem' }}>
                      <div style={{
                        position: 'absolute', left: '-1.75rem', top: '0.375rem',
                        width: '0.5rem', height: '0.5rem', borderRadius: '2px',
                        background: '#fb923c', transform: 'rotate(45deg)', flexShrink: 0,
                      }} />
                      <p style={{ fontSize: '0.6rem', color: 'var(--color-text-faint)', fontFamily: 'monospace', letterSpacing: '0.04em' }}>
                        {formatDateTime(suggestionComment.createdAt)}
                      </p>
                      <p style={{ fontSize: '0.625rem', color: '#fb923c', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: '0.25rem' }}>
                        Demande de modification
                      </p>
                      <p style={{
                        fontSize: '0.8125rem', color: 'var(--color-text)', lineHeight: 1.7,
                        background: 'rgba(249,115,22,0.06)', border: '1px solid rgba(249,115,22,0.2)',
                        padding: '0.5rem 0.75rem', marginTop: '0.375rem',
                      }}>
                  ...
                  {[...internalComments].reverse().map(c => (
                    <div key={c.id} style={{ position: 'relative', marginBottom: '1.25rem' }}>
                      <div style={{
                        position: 'absolute', left: '-1.75rem', top: '0.375rem',
                        width: '0.5rem', height: '0.5rem', borderRadius: '1px',
                        background: 'var(--color-text-faint)', flexShrink: 0,
                      }} />
                      <p style={{ fontSize: '0.6rem', color: 'var(--color-text-faint)', fontFamily: 'monospace', letterSpacing: '0.04em' }}>
                        {formatDateTime(c.createdAt)}
                      </p>
                      <p style={{ fontSize: '0.625rem', color: 'var(--color-text-faint)', textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: '0.25rem' }}>
                        Note interne
                      </p>
                      <p style={{
                        fontSize: '0.8125rem', color: 'var(--color-text)', lineHeight: 1.7,
