...
const filterByTab = (films, tab) => {
  switch (tab) {
    case 'todo':         return films.filter(f => f.status === 'IN_REVIEW');
    case 'modification': return films.filter(f => f.status === 'TO_MODIFY');
    case 'done':         return films.filter(f => ['APPROVED', 'REJECTED'].includes(f.status));
    case 'selection':    return films.filter(f => ['SELECTION', 'FINALIST', 'AWARD'].includes(f.status));
    default:             return films;
  }
};

const STATUS_BADGE = {
  IN_REVIEW:  { background: 'rgba(16,185,129,0.1)',   color: '#10b981', border: '1px solid rgba(16,185,129,0.3)' },
  TO_MODIFY:  { background: 'rgba(249,115,22,0.1)',   color: '#fb923c', border: '1px solid rgba(249,115,22,0.3)' },
  APPROVED:   { background: 'rgba(99,102,241,0.1)',   color: '#818cf8', border: '1px solid rgba(99,102,241,0.3)' },
  REJECTED:   { background: 'rgba(239,68,68,0.1)',    color: '#f87171', border: '1px solid rgba(239,68,68,0.3)' },
  SELECTION:  { background: 'rgba(139,92,246,0.1)',   color: '#a78bfa', border: '1px solid rgba(139,92,246,0.3)' },
  FINALIST:   { background: 'rgba(236,72,153,0.1)',   color: '#ec4899', border: '1px solid rgba(236,72,153,0.3)' },
  AWARD:      { background: 'rgba(245,158,11,0.1)',   color: '#fbbf24', border: '1px solid rgba(245,158,11,0.3)' },
};

const STATUS_LABELS_JURY = {
...
export default function JuryDashboard() {
  ...
  const remaining      = films.filter(f => f.status === 'IN_REVIEW' && (f.votes?.length ?? 0) === 0).length;
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)', color: 'var(--color-text)', fontFamily: 'var(--font-sans)' }}>
      ...
      <main style={{ maxWidth: '72rem', margin: '0 auto', padding: 'clamp(2rem, 4vw, 3rem) clamp(1.5rem, 4vw, 2.5rem)' }}>
        ...
        {/* ── Header éditorial ── */}
        <header style={{ marginBottom: '2rem', paddingBottom: '2rem', borderBottom: '1px solid var(--color-border)' }}>
          ...
          {films.length > 0 && (
            <div style={{ display: 'flex', height: '2px' }}>
              <div style={{ background: '#10b981', flex: voted, transition: 'flex 0.5s' }} />
              <div style={{ background: 'var(--color-border)', flex: remaining || 0.001 }} />
            </div>
          )}
        </header>
        ...
        <div style={{ display: 'flex', gap: '0', marginBottom: '0', borderBottom: '1px solid var(--color-border)' }}>
          {TABS.map(tab => {
            ...
            return (
              ...
              >
                {tab.label}
                <span style={{ marginLeft: '0.5rem', fontSize: '0.625rem', color: isActive ? '#10b981' : 'var(--color-text-faint)' }}>
                  {count}
                </span>
              </button>
            );
        ...
        {/* ── LISTE ── */}
        <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderTop: 'none', overflow: 'hidden' }}>
          {loading ? (
            ...
            <div style={{ padding: '4rem', textAlign: 'center', color: 'var(--color-text-muted)' }}>
              {activeTab === 'all' ? 'Aucun film assigné pour le moment.' : 'Aucun film dans cette catégorie.'}
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse" style={{ minWidth: '580px' }}>
                <thead>
                  <tr style={{ background: 'var(--color-surface-high)', borderBottom: '1px solid var(--color-border)' }}>
                    {['#', 'Statut', 'Film / Pays', 'Réalisateur', 'Note', ''].map((h, i) => (
                      <th
                        key={i}
                        className="label-overline"
                        style={{ padding: '1rem', textAlign: i === 5 ? 'right' : 'left' }}
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {displayedFilms.map((film, i) => {
                    ...
                    const label       = STATUS_LABELS_JURY[film.status] ?? film.status;
                    ...
                    return (
                      ...
                      >
                        {/* # */}
                        <td style={{ padding: '1rem', fontFamily: 'monospace', fontSize: '0.75rem', color: 'var(--color-text-faint)', width: '3rem' }}>
                          {String(i + 1).padStart(2, '0')}
                        </td>

                        {/* Statut */}
                        <td style={{ padding: '1rem' }}>
                          <span style={{ fontSize: '0.6875rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '0.25rem 0.5rem', ...badge }}>
                            {label}
                          </span>
                        </td>
                        ...
                        <td style={{ padding: '1rem', fontSize: '0.875rem', color: 'var(--color-text-muted)' }}>
                          {film.submitter ? `${film.submitter.firstName} ${film.submitter.lastName}` : '—'}
                        </td>

                        {/* Note */}
                        <td style={{ padding: '1rem' }}>
                          {currentVote ? (
                            <span style={{ fontFamily: 'monospace', fontSize: '1rem', fontWeight: 900, color: currentVote.sentiment === 'LIKE' ? '#10b981' : '#ef4444' }}>
                              {currentVote.rating}<span style={{ fontSize: '0.6875rem', color: 'var(--color-text-faint)', fontWeight: 400 }}>/10</span>
                            </span>
                          ) : (
                            <span style={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)', border: '1px solid var(--color-border)', padding: '0.25rem 0.5rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                              À noter
                            </span>
                          )}
                        </td>
