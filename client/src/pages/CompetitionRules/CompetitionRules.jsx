/**
 * CompetitionRules.jsx — MARSAI Festival · Phase 9
 *
 * ┌─────────────────────────────┐
 * │        BANDE HAUTE          │
 * ├──────────────┬──────────────┤
...
const SECTIONS_LEFT = [
  {
    id: '01', slug: 'admissibilite', title: 'Admissibilité',
    content: [
      { type: 'paragraph', text: 'Le concours est ouvert aux créateurs explorant les frontières entre l\u2019intelligence artificielle et la narration cinématographique en une minute exacte.' },
      { type: 'list', items: [
  ...
  {
    id: '02', slug: 'ethique', title: 'Éthique',
    content: [
      { type: 'paragraph', text: 'Chaque soumission doit être accompagnée d\u2019une note détaillant les outils utilisés. Le festival valorise la transparence et le respect des droits d\u2019auteur.' },
    ],
  },
  {
    id: '03', slug: 'propriete', title: 'Propriété',
    content: [
      { type: 'paragraph', text: 'Les participants garantissent être titulaires des droits d\u2019auteur pour les éléments non générés par l\u2019IA.' },
      { type: 'note', text: 'MARSAI se réserve le droit de diffuser les extraits à des fins de promotion.' },
    ],
  },
];
...
const SECTIONS_RIGHT = [
  {
    id: '04', slug: 'soumission', title: 'Soumission',
    content: [
      { type: 'paragraph', text: 'Format requis\u00a0: .MP4 ou .MOV, résolution minimale 1080p.' },
      { type: 'deadline', label: 'Date limite', value: '30 avril 2026' },
    ],
  },
  {
    id: '05', slug: 'selection', title: 'Sélection',
    content: [
...
export default function CompetitionRules() {
  ...
  useGSAP(() => {
    ...
    tl.to(topRef.current,   { opacity: 1, y: 0, duration: 0.70, ease: 'power2.out' }, 0.0);
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 flex-1">
        ...
        >
          ...
          {SECTIONS_LEFT.map((s, i) => (
            <RuleSection key={s.id} section={s} isLast={i === SECTIONS_LEFT.length - 1} />
          ))}
        </div>
        ...
        >
          <div className="w-full bg-gradient-to-r from-[#d1c7a3] to-transparent" style={{ height: '1px', marginBottom: '3rem' }} />
          {SECTIONS_RIGHT.map((s, i) => (
            <RuleSection key={s.id} section={s} isLast={i === SECTIONS_RIGHT.length - 1} />
          ))}
          <p
            className="mt-auto uppercase italic text-white/20"
            style={{ paddingTop: '2rem', borderTop: '1px solid rgba(255,255,255,0.05)', fontSize: '0.55rem', letterSpacing: '0.4em' }}
          >
