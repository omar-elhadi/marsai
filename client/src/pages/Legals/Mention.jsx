...
function MentionSection({ section, isLast }) {
...
export default function Mention() {
  ...
  return (
    <div
      ...
      style={{ paddingTop: 'clamp(4rem,6vw,5rem)', background: '#0a0a0a', position: 'relative', minHeight: '100vh' }}
    >
      {/* ── Image de fond ── */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 flex-1">
          <div ref={leftRef} className="border-b md:border-b-0 md:border-r border-white/10" style={{ padding: 'clamp(3rem,5vw,5rem) clamp(2rem,4vw,4rem)' }}>
            ...
            {SECTIONS_LEFT.map((section, i) => (
              <MentionSection key={section.id} section={section} isLast={i === SECTIONS_LEFT.length - 1} />
            ))}
          </div>
          ...
          <div ref={rightRef} className="flex flex-col" style={{ padding: 'clamp(3rem,5vw,5rem) clamp(2rem,4vw,4rem)' }}>
            ...
            {SECTIONS_RIGHT.map((section, i) => (
              <MentionSection key={section.id} section={section} isLast={i === SECTIONS_RIGHT.length - 1} />
            ))}
