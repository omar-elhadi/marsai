...
const SECTIONS_LEFT = [
  {
    ...
    content: [
      {
        type: 'paragraph',
        text: "Les présentes Conditions d'Utilisation définissent les règles d'accès et d'utilisation du site. En naviguant sur ce site, l'utilisateur accepte pleinement et sans réserve les conditions décrites ci-dessous.",
      },
      {
        type: 'note',
        text: 'Toute navigation vaut acceptation des présentes conditions.',
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'paragraph',
        text: "L'utilisateur s'engage à utiliser le site de manière responsable, légale et respectueuse des autres. Toute utilisation frauduleuse, abusive ou contraire aux lois en vigueur est strictement interdite.",
      },
      {
        type: 'list',
        items: [
          "Respect des droits de propriété intellectuelle du site et de ses partenaires.",
          "Interdiction de toute tentative d'intrusion ou de perturbation des systèmes informatiques.",
        ],
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'paragraph',
        text: "Le site se réserve le droit de modifier, suspendre ou supprimer tout ou partie de ses services à tout moment, sans préavis.",
      },
      {
        type: 'deadline',
        label: 'Mise à jour en vigueur',
        value: 'Mars 2026',
      },
    ],
  },
...
const SECTIONS_RIGHT = [
  {
    ...
    content: [
      {
        type: 'paragraph',
        text: "Les utilisateurs sont invités à consulter régulièrement cette page afin de prendre connaissance des éventuelles mises à jour des présentes conditions.",
      },
      {
        type: 'list',
        items: [
          "L'accès au site peut être restreint temporairement pour des raisons de maintenance.",
          "Certains contenus peuvent nécessiter une authentification préalable.",
        ],
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'paragraph',
        text: "En cas de non-respect des présentes conditions, l'accès au site pourra être restreint ou supprimé sans préavis ni indemnité.",
      },
    ],
  },
  {
    ...
    content: [
      {
        type: 'paragraph',
        text: "Pour toute question relative aux présentes Conditions d'Utilisation, contactez notre équipe via le formulaire officiel du festival.",
      },
      {
        type: 'note',
...
function CGUSection({ section, isLast }) {
...
export default function ConditionsUtilisations() {
  ...
  return (
    ...
    >
      <div ref={topRef} style={{ padding: 'clamp(2rem,3.5vw,3rem) clamp(2rem,5vw,5rem)' }}>
        ...
        <h1 className="text-center font-black uppercase text-white mx-auto leading-none" style={{ fontSize: 'clamp(2rem,6vw,4.5rem)', letterSpacing: '-0.03em' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 flex-1">
        <div ref={leftRef} className="border-b md:border-b-0 md:border-r border-white/10" style={{ padding: 'clamp(3rem,5vw,5rem) clamp(2rem,4vw,4rem)' }}>
          ...
          {SECTIONS_LEFT.map((section, i) => (
            <CGUSection key={section.id} section={section} isLast={i === SECTIONS_LEFT.length - 1} />
          ))}
        </div>
        ...
        <div ref={rightRef} className="flex flex-col" style={{ padding: 'clamp(3rem,5vw,5rem) clamp(2rem,4vw,4rem)' }}>
          ...
          {SECTIONS_RIGHT.map((section, i) => (
            <CGUSection key={section.id} section={section} isLast={i === SECTIONS_RIGHT.length - 1} />
          ))}
