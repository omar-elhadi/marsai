...
const FIELDS = [
  ...
  { key: 'aiToolsUsed', label: 'Outils IA utilisés',   type: 'text',     required: true },
];
...
export default function EditFilmPage() {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'var(--color-bg-pure)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'var(--color-text-muted)' }}>
          ...
          <span style={{ fontFamily: 'var(--font-sans)', fontSize: '0.875rem' }}>Vérification du lien…</span>
        </div>
      </div>
    );
  }
      ...
      <div style={{ minHeight: '100vh', background: 'var(--color-bg-pure)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        <div style={{ maxWidth: '480px', width: '100%', textAlign: 'center' }}>
          ...
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.9rem', color: 'var(--color-text-muted)', lineHeight: 1.6, marginBottom: '2rem' }}>
            {errorMsg || 'Ce lien est expiré ou n\'existe pas.'}
            {' '}Les liens de modification sont valables 7 jours.
          </p>
          <Link
            to="/soumettre"
            style={{
  ...
  if (status === 'success') {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--color-bg-pure)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        <div style={{ maxWidth: '480px', width: '100%', textAlign: 'center' }}>
          <CheckCircle size={40} style={{ color: '#22c55e', margin: '0 auto 1.5rem' }} />
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(1.5rem,4vw,2.2rem)', color: 'var(--color-text)', marginBottom: '0.75rem' }}>
            Modifications envoyées
          </h1>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.9rem', color: 'var(--color-text-muted)', lineHeight: 1.6 }}>
            L'équipe MARSAI a reçu vos modifications et vous recontactera prochainement.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg-pure)', paddingTop: 'clamp(6rem,10vw,8rem)' }}>
      ...
      <section style={{
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(1.5rem,3vw,2.5rem)' }}>
          ...
          {FIELDS.map(({ key, label, type, required }) => (
            <div key={key}>
              <label style={{
                ...
                {label}{required && <span style={{ color: 'var(--color-accent)', marginLeft: '0.2rem' }}>*</span>}
              </label>
          ...
          {saveError && (
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', color: '#f87171' }}>
              {saveError}
            </p>
          )}
          ...
          <div>
            <button
              ...
              style={{
                ...
                textTransform: 'uppercase',
                color:         'var(--color-bg-pure)',
                background:    saving ? 'var(--color-text-muted)' : 'var(--color-text)',
                border:        'none',
                padding:       '1rem 2.5rem',
                borderRadius:  '2px',
