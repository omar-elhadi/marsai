  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ marginLeft: "calc(3.2rem + 20px)" }}
      >
        {/* Carte */}
        <div className="group flex-1 rounded-[var(--radius-sm)] border border-[var(--color-border)] border-l-2 border-l-transparent bg-[var(--color-surface)] px-[clamp(1rem,2vw,1.5rem)] py-[clamp(0.85rem,1.5vw,1.1rem)] transition-[border-color,background,transform] duration-300 ease-[var(--ease-out)] hover:translate-x-[3px] hover:border-l-[var(--color-accent)] hover:bg-[var(--color-surface-high)]">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              <h4 className="title-event mb-[0.3rem]">{event.title}</h4>
              {event.place && (
        ...
        <div className="shrink-0">
          <EventActionButton onClick={onReservation}>
            Réserver
          </EventActionButton>
        </div>
      </div>
