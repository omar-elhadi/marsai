...
const SubmissionStatus = ({ status, uploadProgress, errorMessage }) => {
  /**
    ...
    <div className="mt-8 p-6 bg-black/60 border border-white/10 backdrop-blur-sm">
      ...
      <div className="mb-6">
        <h3 className="text-xl font-bold text-white uppercase tracking-wider">
          ...
          {status === SUBMISSION_STATES.ERROR &&
            "❌ Erreur lors de la soumission"}
          {![SUBMISSION_STATES.SUCCESS, SUBMISSION_STATES.ERROR].includes(
            status,
          ) && "⏳ Soumission en cours..."}
        </h3>
      </div>
          ...
          return (
            ...
            >
              ...
              {index < STEPS.length - 1 && (
                <div
                  className="absolute left-[11px] top-8 w-0.5 h-12 bg-white/20"
      ...
      {status === SUBMISSION_STATES.SUCCESS && (
        <div className="mt-6 p-4 bg-green-500/10 border border-green-500/30 rounded">
          ...
          <p className="text-white/70 text-xs mt-2">
            Vous recevrez un email de confirmation avec un lien pour suivre le
