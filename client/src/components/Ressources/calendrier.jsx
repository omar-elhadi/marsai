...
const EVENT_CATEGORIES = [
  {
    key:      'projections',
    title:    'Projections',
    subtitle: 'Festival du film éalisé en IA',
    date:     '20 Juin 2026',
...
function AccordionItem({ item, globalIndex, isVisible, isOpen, onToggle }) {
  ...
  useGSAP(() => {
    ...
    gsap.to(rowRef.current, { opacity: isVisible ? 1 : 0, x: isVisible ? 0 : 18, duration: 0.45, ease: 'power2.out' });
  }, [isVisible]);
    ...
    <div ref={rowRef} className="accordion-row">
      <button onClick={onToggle} className="accordion-btn">
        <div className="accordion-btn-left">
          <span className="accordion-num">{String(globalIndex + 1).padStart(2, '0')}</span>
          <span className={"accordion-title" + (isOpen ? " is-open" : "")}>{item.title}</span>
        </div>
        <span className={"accordion-toggle" + (isOpen ? " is-open" : "")}>{isOpen ? "−" : "+"}</span>
      </button>
  ...
  return (
    <div className="category-block">
      ...
      {category.items.map((item, i) => (
        <AccordionItem
          key={item.id} item={item} globalIndex={startIndex + i}
...
export default function Calendrier() {
  ...
  useGSAP(() => {
    ...
    ScrollTrigger.create({
      ...
      onEnter() {
        ...
        tl.to([line1Ref.current, line2Ref.current], { yPercent: 0, duration: 0.85, stagger: 0.12, ease: 'power3.out' }, 0.15);
        tl.to(subtitleRef.current, { opacity: 1, y: 0, duration: 0.65, ease: 'power2.out' }, 0.50);
      },
    });
  }, { scope: sectionRef });
  ...
  useEffect(() => {
    ...
    setVisibleCount(0);
    let current = 0;
    const id = setInterval(() => { current++; setVisibleCount(current); if (current >= total) clearInterval(id); }, 280);
  ...
  useGSAP(() => {
    ...
    const tl = gsap.timeline({
      scrollTrigger: { trigger: timelineRef.current, start: 'top center', end: 'bottom center', scrub: 0.4 },
    });
