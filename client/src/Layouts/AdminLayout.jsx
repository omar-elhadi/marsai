...
function AdminLayout() {
  ...
  return (
    <div
      ...
      style={{ background: 'var(--color-bg-pure)', color: 'var(--color-text)', fontFamily: 'var(--font-sans)' }}
    >
      ...
      <aside
        ...
        `}
        style={{ background: 'var(--color-bg-pure)', borderRight: '1px solid var(--color-border)' }}
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.875rem' }}>
            <span style={{ width: '2rem', height: '1px', background: 'var(--color-accent)', flexShrink: 0 }} />
        ...
        {!isMobileMenuOpen && (
          <button
            ...
            onMouseLeave={e => { e.currentTarget.style.background = 'var(--color-surface)'; e.currentTarget.style.color = 'var(--color-text-faint)'; }}
          >
          ...
          {[
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon
                    size={isCollapsed && !isMobileMenuOpen ? 24 : (isMobileMenuOpen ? 28 : 22)}
                    strokeWidth={isActive ? 2.5 : 1.75}
                    style={{ color: isActive ? '#6366f1' : 'var(--color-text-muted)', flexShrink: 0, transition: 'color 0.2s' }}
        ...
        {/* Déconnexion */}
        <div
          style={{ padding: 'clamp(1.5rem, 3vw, 2rem)', borderTop: '1px solid var(--color-border)', flexShrink: 0, display: 'flex', justifyContent: isCollapsed && !isMobileMenuOpen ? 'center' : 'flex-start' }}
        >
          <button
            onClick={handleLogout}
            className={`flex items-center gap-3 transition-all outline-none group
              ${isMobileMenuOpen ? 'text-lg py-4 px-4 gap-5' : 'text-xs'}`}
            style={{ fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.15em', color: 'var(--color-text-muted)', background: 'none', border: 'none', cursor: 'pointer', transition: 'color 0.2s' }}
            onMouseEnter={e => e.currentTarget.style.color = '#6366f1'}
            onMouseLeave={e => e.currentTarget.style.color = 'var(--color-text-muted)'}
          >
            <LogOut
              size={isCollapsed && !isMobileMenuOpen ? 20 : (isMobileMenuOpen ? 26 : 18)}
      ...
      >
        <div style={{ padding: 'clamp(1.5rem, 3vw, 2.5rem)', maxWidth: '88rem', margin: '0 auto', minHeight: '100vh' }}>
          <Outlet />
        </div>
      </main>
