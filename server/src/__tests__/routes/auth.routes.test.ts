  ...
  describe('POST /api/auth/login', () => {
    it('should return 200 and user info on success', async () => {
      ...
      vi.mocked(authService.loginAdmin).mockResolvedValue({
        token: 'test-token',
        user: { id: 'u1', email: 'test@marsai.com', role: 'ADMIN', firstName: 'A', lastName: 'B' }
      });
    ...
    it('should return 401 on invalid credentials', async () => {
      vi.mocked(authService.loginAdmin).mockRejectedValue(new Error('Identifiants incorrects.'));
