...
describe('Film Service', () => {
  describe('changeFilmStatus', () => {
    it('should successfully change status when transition is valid (SUBMITTED -> IN_REVIEW)', async () => {
      // @ts-ignore
      prismaMock.film.findUnique.mockResolvedValue({ id: '1', status: 'SUBMITTED' });
      // @ts-ignore
      prismaMock.film.update.mockResolvedValue({ id: '1', status: 'IN_REVIEW' });

      const result = await filmService.changeFilmStatus('1', 'IN_REVIEW', 'MODERATOR');
    ...
    it('should throw an error if transition is invalid (SUBMITTED -> SELECTION)', async () => {
      ...
      await expect(filmService.changeFilmStatus('1', 'SELECTION', 'ADMIN'))
    ...
    it('should block non-admin from changing status to SELECTION', async () => {
      ...
      await expect(filmService.changeFilmStatus('1', 'SELECTION', 'MODERATOR'))
        .rejects.toThrow('La transition vers SELECTION est réservée aux administrateurs.');
    });
    ...
    it('should allow admin to change status to SELECTION from APPROVED', async () => {
      ...
      prismaMock.film.findUnique.mockResolvedValue({ id: '1', status: 'APPROVED' });
      // @ts-ignore
      prismaMock.film.update.mockResolvedValue({ id: '1', status: 'SELECTION' });

      const result = await filmService.changeFilmStatus('1', 'SELECTION', 'ADMIN');
    ...
    it('should throw 404 if film is not found', async () => {
      ...
      await expect(filmService.changeFilmStatus('bad_id', 'IN_REVIEW', 'ADMIN'))
