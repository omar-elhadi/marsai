 ...
 */
export const extractVideoMetadata = async (input) => {
  let tempFilePath = null;
  let isBuffer = Buffer.isBuffer(input);

  try {
    // Si c'est un buffer, créer un fichier temporaire
    if (isBuffer) {
    ...
    return new Promise((resolve, reject) => {
      ffmpeg.ffprobe(input, (err, metadata) => {
        ...
        if (err) {
          cleanup();
          return reject(
            new Error(`Impossible d'analyser la vidéo : ${err.message}`),
          );
        }
        ...
        try {
          ...
          if (!videoStream) {
            cleanup();
            return reject(
              new Error("Aucun stream vidéo trouvé dans le fichier"),
            );
          }
          ...
          reject(
            new Error(
              `Erreur lors du parsing des métadonnées : ${parseError.message}`,
            ),
          );
        }
      });
    ...
    // Nettoyer en cas d'erreur lors de la création du fichier temp
    if (tempFilePath && fs.existsSync(tempFilePath)) {
      fs.unlinkSync(tempFilePath);
    }
    throw error;
...
export const validateVideoFile = async (file) => {
  ...
  try {
    ...
    // if (
    //   metadata.width < VIDEO_CONSTRAINTS.MIN_WIDTH ||
    //   metadata.height < VIDEO_CONSTRAINTS.MIN_HEIGHT
    // ) {
    //   errors.push(
    ...
    if (
      metadata.codec &&
      !VIDEO_CONSTRAINTS.ALLOWED_CODECS.includes(metadata.codec.toLowerCase())
    ) {
      errors.push(
